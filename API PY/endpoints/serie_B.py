...
serieB_bp = Blueprint("serie_B", __name__, url_prefix="/Serie_B")
...
def atualizar_clube_serie_b(idSerieB):
    dados = request.get_json(silent=True)
    if not dados:
        abort(400, description="JSON inválido ou ausente")
...
def deletar_clube_serie_b(idSerieB):
    conn = conectar()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM Serie_B WHERE idSerieB = ?", (idSerieB,))
    conn.commit()
